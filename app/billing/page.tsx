...
import { Check, Zap, Settings, Loader2 } from 'lucide-react'; // Új importok
...
export default function BillingPage() {
  ...
  const [portalLoading, setPortalLoading] = useState(false); // Új state
  ...
  // ÚJ FÜGGVÉNY: Stripe Portal megnyitása
  ...
  if (loading) return <div className="p-8 italic opacity-50 text-white">Loading...</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-12 p-8">
      <header>
        <h1 className="text-4xl font-black tracking-tight mb-2 text-white">Billing & Plan</h1>
        ...
        <div className="space-y-6">
          <div className="bg-blue-600 rounded-[32px] p-8 text-white shadow-xl">
            <div className="flex justify-between items-start mb-8">
              <div>
                <p className="text-blue-100 text-xs font-black uppercase mb-2">Aktív csomagod</p>
            ...
            <button 
              ...
              className="w-full py-4 bg-white text-blue-600 rounded-2xl font-black hover:bg-blue-50 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
              {portalLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Settings className="w-5 h-5" />}
              Számlázás és kártyák kezelése
            </button>
          </div>
        </div>
      ) : (
