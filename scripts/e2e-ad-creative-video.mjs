...
// Deprecated: use scripts/e2e-ad-creative-video.ts (run via `npx tsx ...`).
process.exit(1);
...
async function uploadVideoToStorage({ clientId, userId, jobId, videoUrl }) {
  const r = await fetch(videoUrl);
  if (!r.ok) throw new Error(`Video download failed (${r.status})`);
  const bytes = await r.arrayBuffer();
  const bucket = "brand-assets";
  const storagePath = `ad-creative-videos/${clientId}/${userId}/e2e/${encodeURIComponent(jobId)}.mp4`;
  ...
  return { bucket, path: storagePath, signedUrl: signed.signedUrl, bytes: bytes.byteLength };
}
...
async function main() {
  ...
  const referenceImageUrl =
    "https://upload.wikimedia.org/wikipedia/commons/7/77/Coca-Cola_can_355ml.jpg";
  ...
  for (const c of concepts) {
    ...
    if (!r.ok) {
      ...
      const qc = r.asset.qc;
      console.log(
        `- ${c.angleId}: OK url=${Boolean(r.asset.url)} qc=${qc ? `${qc.realism_score}/10 realism, ${qc.artifact_score}/10 artifacts, ${qc.brand_consistency_score}/10 brand (retries=${qc.retry_count})` : "none"}`
      );
    }
  }
  ...
  // Basic image accessibility check (provider signed URL)
  ...
  if (started.status === "completed") {
    ...
    for (let i = 0; i < 60; i++) {
      await sleep(2000);
      const polled = await pollAdCreativeVideoJob({ job_id: jobId, source_image_url: best.asset.url });
      if (!polled.ok) {
  ...
  // Upload to Supabase storage (emulates API finalize step) using dummy ids.
